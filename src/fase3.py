import random
import math
from fase1_algoritmos import busqueda_ucs

def generar_matriz_entregas(grafo, puntos_entrega):
    """Calcula el costo real de viaje entre todos los puntos usando UCS."""
    n = len(puntos_entrega)
    matriz = [[0] * n for _ in range(n)]
    for i in range(n):
        for j in range(n):
            if i != j:
                _, _, _, costo = busqueda_ucs(grafo, puntos_entrega[i], puntos_entrega[j])
                matriz[i][j] = costo
    return matriz

def calcular_costo_ruta(ruta, matriz):
    """Suma las distancias de la permutación, incluyendo el regreso al origen."""
...
def algoritmo_genetico(poblacion, matriz, gen, prob_mut):
    ...
    for _ in range(gen):
        ...
        while len(nueva) < len(poblacion):
            # Selección truncada simple para los padres
